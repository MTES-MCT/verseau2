...
import { getDateAsISODate } from '@lib/shared';
import {
...
interface ConformiteSteuListQueryRow {
...
interface ConformiteSclListQueryRow {
...
interface ConformiteSteuDetailQueryRow {
...
interface ConformiteSclDetailQueryRow {
  volume_deverse_periode_pc: number | string | null;
  volume_deverse_annee_pc: number | string | null;
  conformite_volume_periode: number | string | null;
  conformite_volume_annee: number | string | null;
  flux_deverse_periode_pc: number | string | null;
  flux_deverse_annee_pc: number | string | null;
  conformite_flux_periode: number | string | null;
  conformite_flux_annee: number | string | null;
  jours_deversement_periode_nb: number | string | null;
  jours_deversement_annee_nb: number | string | null;
  conformite_jours_deversement_periode: number | string | null;
  conformite_jours_deversement_annee: number | string | null;
}
...
export class RoseauRepository implements RoseauGateway {
  ...
  private async queryRows<T>(query: string, params: ReadonlyArray<string | number>): Promise<T[]> {
  ...
  async findConformiteSteu(filters: ConformiteSteuFilters): Promise<{ data: ConformiteSteuRow[]; total: number }> {
    const { steuCdns, trancheObligationLibelle, impact, page, pageSize, sortBy } = filters;
    const annee = new Date().getFullYear();

    const steuCdnsPlaceholders = steuCdns.map((_, i) => `$${i + 2}`).join(', ');
    const params: (string | number)[] = [annee, ...steuCdns];

    let whereClause = `WHERE steu.steu_cdn IN (${steuCdnsPlaceholders})`;

    if (trancheObligationLibelle) {
      whereClause += ` AND tltobl.tltobl_lb = $${params.length + 1}`;
      params.push(trancheObligationLibelle);
    }

    let impactCondition = '';
    if (impact === 'avec') {
      impactCondition = ` AND (cfprf.cfprf_r_glob_perf_eru_conf_in <> steureg.steureg_per_nat_conf_in OR cfprf.cfprf_r_glob_perf_loc_conf_in <> steureg.steureg_per_loc_conf_in)`;
    } else if (impact === 'sans') {
      impactCondition = ` AND NOT (cfprf.cfprf_r_glob_perf_eru_conf_in <> steureg.steureg_per_nat_conf_in OR cfprf.cfprf_r_glob_perf_loc_conf_in <> steureg.steureg_per_loc_conf_in)`;
    }
    whereClause += impactCondition;

    const baseQuery = `
      WITH params AS (SELECT $1::int AS annee)
      FROM roseau.steu
      ...
      JOIN roseau.aga ON aga.zgc_cdn = cxn.amont_zgc_cdn
      JOIN roseau.trobl ON trobl.aga_cdn = aga.aga_cdn
      ...
      JOIN roseau.tltobl ON tltobl.tltobl_rfa = trobl.tltobl_rfa
      JOIN roseau.cpy ON cpy.steu_cdn = steu.steu_cdn
      ...
      JOIN roseau.cfprf ON cfprf.steu_cdn = steu.steu_cdn
      ...
      JOIN roseau.steureg ON steureg.steu_cdn = steu.steu_cdn
        AND steureg.steureg_an = (SELECT annee FROM params)
      ${whereClause}
    `;

    const [countRow] = await this.queryRows<CountRow>(`SELECT COUNT(*) as total ${baseQuery}`, params);
    ...
    const sortCol = sortBy ? sortMap[sortBy] || 'ouvrage_depollution_code' : 'ouvrage_depollution_code';

    const dataRows = await this.queryRows<ConformiteSteuListQueryRow>(
      `
      SELECT
    ...
    const data: ConformiteSteuRow[] = dataRows.map((r) => ({
      ouvrageDepollutionCode: r.ouvrage_depollution_code,
      ouvrageDepollutionNom: r.ouvrage_depollution_nom,
      trancheObligationLibelle: r.tranche_obligation_libelle,
      capaciteNominaleEH: this.toNullableNumber(r.capacite_nominale_eh),
      suiviDebutDate: r.suivi_debut_date ? getDateAsISODate(new Date(r.suivi_debut_date)) : null,
      suiviFinDate: r.suivi_fin_date ? getDateAsISODate(new Date(r.suivi_fin_date)) : null,
      conformiteNationaleProvisoire: r.conformite_nationale_provisoire,
      conformiteLocaleProvisoire: r.conformite_locale_provisoire,
      impactConformite: r.impact_conformite,
      suiviRegulierEffectue: r.suivi_regulier_effectue,
      suiviRegulierDate: r.suivi_regulier_date ? getDateAsISODate(new Date(r.suivi_regulier_date)) : null,
    }));
  ...
  async findConformiteScl(filters: ConformiteSclFilters): Promise<{ data: ConformiteSclRow[]; total: number }> {
    ...
    let impactCondition = '';
    if (impact === 'avec') {
      impactCondition = ` AND (bil.bilanscl_conf_tp_eru_in <> sclreg.sclreg_per_nat_tp_conf_in)`;
    } else if (impact === 'sans') {
      impactCondition = ` AND NOT (bil.bilanscl_conf_tp_eru_in <> sclreg.sclreg_per_nat_tp_conf_in)`;
    }
    whereClause += impactCondition;
    ...
    const baseQuery = `
      WITH params AS (SELECT $1::int AS annee)
      FROM roseau.scl
      JOIN roseau.steu ON steu.steu_cdn = scl.steu_cdn
      JOIN roseau.cxntech cxn ON cxn.aval_steu_cdn = steu.steu_cdn
        AND cxn.amont_zgc_cdn IS NOT NULL
        AND DATE_PART('year', cxn.cxntech_creation_dt) <= (SELECT annee FROM params)
        AND (cxn.cxntech_retrait_dt IS NULL OR DATE_PART('year', cxn.cxntech_retrait_dt) >= (SELECT annee FROM params))
      ...
      JOIN roseau.sclreg ON sclreg.scl_cdn = scl.scl_cdn
    ...
    const sortCol = sortBy ? sortMap[sortBy] || 'systeme_collecte_code' : 'systeme_collecte_code';

    const dataRows = await this.queryRows<ConformiteSclListQueryRow>(
      `
      SELECT
    ...
    const data: ConformiteSclRow[] = dataRows.map((r) => ({
      systemeCollecteCode: r.systeme_collecte_code,
      systemeCollecteNom: r.systeme_collecte_nom,
      trancheObligationLibelle: r.tranche_obligation_libelle,
      typeScl: r.type_scl,
      suiviDebutDate: r.suivi_debut_date ? getDateAsISODate(new Date(r.suivi_debut_date)) : null,
      suiviFinDate: r.suivi_fin_date ? getDateAsISODate(new Date(r.suivi_fin_date)) : null,
      conformiteLocaleTempsPluieProvisoire: r.conformite_locale_temps_pluie_provisoire,
      conformiteNationaleTempsPluieProvisoire: r.conformite_nationale_temps_pluie_provisoire,
      impactConformite: r.impact_conformite,
      suiviRegulierEffectue: r.suivi_regulier_effectue,
      suiviRegulierDate: r.suivi_regulier_date ? getDateAsISODate(new Date(r.suivi_regulier_date)) : null,
    }));
  ...
  async findConformiteSteuDetail(steuCdn: number, annee: number): Promise<ConformiteSteuDetailRow | null> {
    const [row] = await this.queryRows<ConformiteSteuDetailQueryRow>(
      `
      SELECT
    ...
    return {
      conformiteLocaleParametresConformesPeriodeNb: this.toNullableNumber(row.conf_loc_per_nb),
      conformiteLocaleParametresConformesAnneeNb: this.toNullableNumber(row.conf_loc_an_nb),
      nonConformiteLocaleParametresConformesPeriodeNb: this.toNullableNumber(row.non_conf_loc_per_nb),
      nonConformiteLocaleParametresConformesAnneeNb: this.toNullableNumber(row.non_conf_loc_an_nb),
      redhLocaleParametresConformesPeriodeNb: this.toNullableNumber(row.redh_loc_per_nb),
      redhLocaleParametresConformesAnneeNb: this.toNullableNumber(row.redh_loc_an_nb),
      conformiteLocaleParametresConformesPeriodeLibelle: row.conf_loc_per_lb,
      conformiteLocaleParametresConformesAnneeLibelle: row.conf_loc_an_lb,
      nonConformiteLocaleParametresConformesPeriodeLibelle: row.non_conf_loc_per_lb,
      nonConformiteLocaleParametresConformesAnneeLibelle: row.non_conf_loc_an_lb,
      redhLocaleParametresConformesPeriodeLibelle: row.redh_loc_per_lb,
      redhLocaleParametresConformesAnneeLibelle: row.redh_loc_an_lb,
      conformiteNationaleParametresConformesPeriodeNb: this.toNullableNumber(row.conf_nat_per_nb),
      conformiteNationaleParametresConformesAnneeNb: this.toNullableNumber(row.conf_nat_an_nb),
      nonConformiteNationaleParametresConformesPeriodeNb: this.toNullableNumber(row.non_conf_nat_per_nb),
      nonConformiteNationaleParametresConformesAnneeNb: this.toNullableNumber(row.non_conf_nat_an_nb),
      redhNationaleParametresConformesPeriodeNb: this.toNullableNumber(row.redh_nat_per_nb),
      redhNationaleParametresConformesAnneeNb: this.toNullableNumber(row.redh_nat_an_nb),
      conformiteNationaleParametresConformesPeriodeLibelle: row.conf_nat_per_lb,
      conformiteNationaleParametresConformesAnneeLibelle: row.conf_nat_an_lb,
      nonConformiteNationaleParametresConformesPeriodeLibelle: row.non_conf_nat_per_lb,
      nonConformiteNationaleParametresConformesAnneeLibelle: row.non_conf_nat_an_lb,
      redhNationaleParametresConformesPeriodeLibelle: row.redh_nat_per_lb,
      redhNationaleParametresConformesAnneeLibelle: row.redh_nat_an_lb,
      hcnfPeriodeNb: this.toNullableNumber(row.hcnf_per_nb),
      hcnfAnneeNb: this.toNullableNumber(row.hcnf_an_nb),
      hctsPeriodeNb: this.toNullableNumber(row.hcts_per_nb),
      hctsAnneeNb: this.toNullableNumber(row.hcts_an_nb),
      hcnfPeriodeLibelle: row.hcnf_per_lb,
      hcnfAnneeLibelle: row.hcnf_an_lb,
      hctsPeriodeLibelle: row.hcts_per_lb,
      hctsAnneeLibelle: row.hcts_an_lb,
      evtPeriodeNb: this.toNullableNumber(row.evt_per_nb),
      evtAnneeNb: this.toNullableNumber(row.evt_an_nb),
    };
  }

  async findConformiteSclDetail(sclCdn: number, annee: number): Promise<ConformiteSclDetailRow | null> {
    const [row] = await this.queryRows<ConformiteSclDetailQueryRow>(
      `
      SELECT
    ...
    return {
      volumeDeversePeriodePc: this.toNullableNumber(row.volume_deverse_periode_pc),
      volumeDeverseAnneePc: this.toNullableNumber(row.volume_deverse_annee_pc),
      conformiteVolumePeriode: this.toNullableNumber(row.conformite_volume_periode),
      conformiteVolumeAnnee: this.toNullableNumber(row.conformite_volume_annee),
      fluxDeversePeriodePc: this.toNullableNumber(row.flux_deverse_periode_pc),
      fluxDeverseAnneePc: this.toNullableNumber(row.flux_deverse_annee_pc),
      conformiteFluxPeriode: this.toNullableNumber(row.conformite_flux_periode),
      conformiteFluxAnnee: this.toNullableNumber(row.conformite_flux_annee),
      joursDeversementPeriodeNb: this.toNullableNumber(row.jours_deversement_periode_nb),
      joursDeversementAnneeNb: this.toNullableNumber(row.jours_deversement_annee_nb),
      conformiteJoursDeversementPeriode: this.toNullableNumber(row.conformite_jours_deversement_periode),
      conformiteJoursDeversementAnnee: this.toNullableNumber(row.conformite_jours_deversement_annee),
    };
  }
